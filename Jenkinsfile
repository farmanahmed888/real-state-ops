pipeline {
    ...
    tools{
     ...
     environment {
        ...
        DOCKER_USERNAME = 'farmanahmed888'
        DOCKER_PASSWORD = 'farmanahmed888'
    }
    stages {
        stage('Git Checkout') {
            steps {
                git 'https://github.com/farmanahmed888/full-stack-estate-main.git'
            }
        }
    ...
    stage('Sonarqube') {
     ...
     stage('Docker build, tag, and push') {
            steps {
                script {
                    // Docker credentials
                    def DOCKER_USERNAME = 'farmanahmed888'
                    def DOCKER_PASSWORD = 'farmanahmed888'
